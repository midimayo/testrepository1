...
public class LoadScenesA : MonoBehaviour 
{
    ...
    public void LoadA(string main_scene)
    {
        Debug.Log("main_scene to load:" + main_scene);
        SceneManager.LoadScene(main_scene);

    }
}
//reload the scene if u on collision
///private void OnCollisionEnter2D(Collision2D col)
///{
   /// if (col.gameObject.name == "Pink Monster")
   /// {
...
public class PlatformerController : MonoBehaviour
{
    ...
    void Update()
    {
     //character animator checking for speed
        ...
        if (Input.GetKeyDown(KeyCode.Space) && canJump == true)
        {
            ...
            //character animator checking for jump
            char_animator.SetBool("isJumping", true);
        }
        else
        {
            canJump = false;
    ...
    {
        char_animator.SetBool("IsJumping", false);
    }
